    ...
    return (
      ...
      >
        <div className="relative flex min-h-[60vh] flex-col items-center justify-center text-center">
          ...
          <h1 className="editorial mt-6 text-5xl text-cream sm:text-6xl">
            Somu's Secret Vault
          </h1>

          <p className="mt-4 max-w-xs text-sm text-cream/60">
            Only a true Swiftie can open this.
          </p>
    ...
    >
      <div className="relative">
        ...
        <header className="mb-8">
          ...
          <h1 className="editorial mt-3 text-4xl text-cream sm:text-5xl">
            The Vault
          </h1>
        </header>
        ...
        <div className="mb-10 space-y-3">
          {unlocked.map((i) => {
            ...
            return (
              <div
                key={i}
                className="anim-scale-in border border-gold/35 bg-cream/[0.04] p-4"
              >
                <p className="overline text-[0.5rem] text-gold">
                  Correct! You unlocked
                </p>

                <p className="editorial mt-2 text-2xl text-cream">
                  {item.reward}
                </p>
        ...
        {done ? (
          <Reveal>
            <div className="border border-gold/40 p-6 text-center">
              <p className="editorial text-3xl text-gold">
                Vault emptied.
              </p>
          ...
          q && (
            <form
              onSubmit={submit}
              className="border border-chrome/20 bg-cream/[0.03] p-5"
            >
              <p className="overline text-[0.5rem] text-chrome">
                Riddle {index + 1}
              </p>

              <p className="editorial mt-3 text-2xl text-cream">
                {q.prompt}
              </p>
              ...
              {status === "wrong" && (
                <p className="hand anim-fade mt-3 text-lg text-rose">
                  not quite. try again, superstar. 😭
                </p>
              )}
              ...
              {showHint && (
                <p className="hand mt-4 text-lg text-gold/80">
                  hint: {q.hint}
                </p>
              )}
            </form>
        ...
        {!done && unlocked.includes(index) && (
          <div className="mt-6">
            <SoftButton
              variant="gold"
              onClick={() => {
                const next = VAULT_QUESTIONS.findIndex(
