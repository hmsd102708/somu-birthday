...
export function CuteSomu({ onBack }: { onBack: () => void }) {
  const notes = [
    ...
    {
      t: "Exhibit C",
      b: "Claims innocence. Has never been innocent. [ADD MEMORY HERE]",
    },
  ];

  const photos: PhotoId[] = ["cute-1", "cute-2", "cute-3"];

  return (
    <SectionShell onBack={onBack} label="Cute Somu" tone="pink">
      <header className="mb-8">
        <p className="overline text-[0.6rem] opacity-60">
          Rare footage
        </p>

        <h1 className="editorial mt-3 text-5xl sm:text-7xl">
          Cute Somu
        </h1>
      ...
      {/* THREE INDEPENDENT CUTE SOMU PHOTOS */}
      <div className="grid gap-6 sm:grid-cols-3">
        {photos.map((photo, i) => (
          <Reveal key={photo} delay={i * 90}>
            <Photo
              id={photo}
              shape="polaroid"
              tilt={i === 1 ? 1.8 : -1.8}
        ...
        {notes.map((n, i) => (
          <Reveal key={n.t} delay={i * 70}>
            <div className="border-l-2 border-wine/30 pl-4">
              <p className="overline text-[0.5rem] opacity-60">
                {n.t}
              </p>
...
export function ArtistSomu({ onBack }: { onBack: () => void }) {
  return (
    <SectionShell
      onBack={onBack}
      label="Generational Artist"
      tone="dark"
    >
      <header className="mb-8">
        <p className="overline text-[0.6rem] text-chrome">
          Cover story
        </p>
      ...
      <div className="mt-10 space-y-6">
        ...
        <Reveal delay={80}>
          <p className="text-sm leading-relaxed text-cream/70">
            Sold-out shows in the festivals. Phenomenal performances ever
            in history. Critics (me) call it "the best voice I've ever
            been personally serenaded by."
          </p>
        </Reveal>
      </div>
    </SectionShell>
